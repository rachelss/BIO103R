...
### 3c. Show the variation in the data using boxplots

```{r head5, eval=FALSE}
# Plot the data as boxplots using geom_boxplot
...
```{r head3, eval=FALSE}
# Make your plot for the plant_data_means data
...
* Add ` + geom_errorbar(aes(ymin=mean+stdev, ymax=mean-stdev))` to your plot
...
### 5e. Draw lines representing a model that fits your data
...
* Add ` + geom_smooth(method="lm", se = FALSE)` to your plot
...
## 6. Fit a model to represent the change in seedling height over time
...
## 7. Compare height under different conditions using statistics (a t-test)
...
1. Summary table for your section’s data ( _wide form_, made in excel/sheets/or R). Do not include all the raw data, just means and standard deviations.
2. Boxplot of all data from Step 3c.
3. Graph with mean data points and error bars from Step 5d.
4. Results of your t-tests from Step 6 in table form.
5. Your code. Code should be organized, include good comments, and include only code that works and does not produce errors.
6. Answers to the following questions (one full page, double-spaced):
